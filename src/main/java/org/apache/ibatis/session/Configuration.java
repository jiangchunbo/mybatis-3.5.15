...
public class Configuration {
  ...
  /**
   * MyBatis 对 ConcurrentHashMap 进行了继承，并增加了一些功能
   * <p>
   * 一般比如用于存储 MappedStatement ResultMap
   * <p>
   * put 已有 key 会报错
   * <p>
   * put 冲突，支持回调函数提醒
   ...
   * 获取不到 key 报错
   * <p>
   * 支持 short name 与 long name 共同存储
   *
  ...
  protected static class StrictMap<V> extends ConcurrentHashMap<String, V> {
    ...
    /**
     * 调用 put 方法，发生冲突时的回调函数，比如可以用于通知开发者
     */
    ...
    public V put(String key, V value) {
      ...
      // 这是因为 put 支持 short key
      // 一般用于，传入了一个带有 namespace 的 key，这是一个 long key

      if (key.contains(".")) {
        // 获得 short name，放到 map 里
        final String shortKey = getShortName(key);
        if (super.get(shortKey) == null) {
          super.put(shortKey, value);
        } else {
          // 万一 short name 已经存在了，那么就放一个类型是 Ambiguity 的对象。表示模棱两可
    ...
    public V get(Object key) {
      ...
      // 若通过 key（一般是 short key），发现对象是 Ambiguity
      // 说明是发生冲突了
      // 这里还有点惰性失败的意思
